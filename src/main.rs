  ...
  \___||_||_|\__,_| |_||_||_| |_| |_||_||_| _|\_\  
    ...
    match &args.command {
        ...
        Some(Command::GetChainReferenceFeeds {chain}) => {
            ...
            let price_feeds = DataFeeds::load_chain_feeds(chain).await;
              
        },
        Some(Command::GetOracle { chain, token, base }) => {
            let chain = Chain::from_str(chain).expect("chain not found");
            let price_feeds = DataFeeds::with_initial_values();
            if let Some(oracle) = price_feeds.get_oracle(chain, &(token.to_uppercase()), &(base.to_uppercase())) { 
                println!("{:#?}", oracle)
            } else {
        ...
        Some(Command::GetLatestAnswer { chain, token, base }) => {
            let chain = Chain::from_str(chain).expect("chain not found");
            if token.len() == 0 || token.len() != base.len() {
                ...
                match token.len() {
                    ...
                    _ => {
                        ...
                        for (t,b) in token.into_iter().zip(base) {
                            ...
                            if let Some(oracle) = price_feeds.get_oracle(chain, &tt, &bb) {
                                all_oracles.push(oracle);
                                tokens.push(tt);
                                bases.push(bb);
                                let data = oracle.latest_answer_call();
                                all_queries.push(
                                    Call3 {
                            ...
                            Ok(r) => {
                                ...
                                for (((t,b) , o), r) in tokens.into_iter().zip(bases).zip(all_oracles).zip(results) {
                                    if r.success {
                                        ...
                                        println!("Latest answer for {}: {} {}", t, format_units(val, o.decimals).unwrap(), b)
                                    } else {
            ...
            if let Ok(provider) = get_provider(chain, &rpc_url_id) {
                match round_id.len() {
                    ...
                    _ => {
                        ...
                        if let Some(oracle) = price_feeds.get_oracle(chain, &token, &base) {
                            for rid in round_id.into_iter() {
                                    let data = oracle.get_round_data_call(rid.clone());
                                    all_queries.push(
                                        Call3 {
                                            target: oracle.address,
                                            allowFailure: true,
                                ...
                                Ok(r) => {
                                    ...
                                    for result in results.into_iter() {
                                        if result.success {
                                            ...
                                            println!("Error getting latest answer for {}/{}", token, base)
                                        }
                                    }
                                },
                                Err(e) => println!("Err {:#?}", e)
